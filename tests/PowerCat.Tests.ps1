...
"@
  ...
  Context "QOL feature tests" {
    It "Accepts comma-separated -Extensions string" {
      $tempDir = Join-Path ([System.IO.Path]::GetTempPath()) "PowerCatCommaExt_$([System.Guid]::NewGuid())"
      New-Item -ItemType Directory -Path $tempDir -Force | Out-Null
      Set-Content -Path (Join-Path $tempDir "one.ps1") -Value "PS1"
      Set-Content -Path (Join-Path $tempDir "two.md") -Value "MD"
      $outFile = Join-Path ([System.IO.Path]::GetTempPath()) "commaext_$([System.Guid]::NewGuid()).txt"
      try {
        Invoke-PowerCat -s $tempDir -o $outFile -Extensions ".ps1,.md"
        $content = Get-Content $outFile -Raw
        $content | Should -Match "PS1"
        $content | Should -Match "MD"
      }
      finally {
        Remove-Item -Path $outFile -Force -ErrorAction SilentlyContinue
        Remove-Item -Path $tempDir -Recurse -Force -ErrorAction SilentlyContinue
    ...
    It "Respects -ExcludeExtensions to remove selected types" {
      $tempDir = Join-Path ([System.IO.Path]::GetTempPath()) "PowerCatExcludeExt_$([System.Guid]::NewGuid())"
      New-Item -ItemType Directory -Path $tempDir -Force | Out-Null
      Set-Content -Path (Join-Path $tempDir "one.ps1") -Value "PS1"
      Set-Content -Path (Join-Path $tempDir "two.md") -Value "MD"
      $outFile = Join-Path ([System.IO.Path]::GetTempPath()) "excludeext_$([System.Guid]::NewGuid()).txt"
      try {
        Invoke-PowerCat -s $tempDir -o $outFile -Extensions ".ps1", ".md" -ExcludeExtensions ".md"
        $content = Get-Content $outFile -Raw
        $content | Should -Match "PS1"
        $content | Should -Not -Match "MD"
      }
      finally {
        Remove-Item -Path $outFile -Force -ErrorAction SilentlyContinue
        Remove-Item -Path $tempDir -Recurse -Force -ErrorAction SilentlyContinue
    ...
    It "ForceOverwrite removes existing read-only output file when specified" {
      $tempDir = Join-Path ([System.IO.Path]::GetTempPath()) "PowerCatForce_$([System.Guid]::NewGuid())"
      New-Item -ItemType Directory -Path $tempDir -Force | Out-Null
      Set-Content -Path (Join-Path $tempDir "one.md") -Value "NEW"
      $outFile = Join-Path ([System.IO.Path]::GetTempPath()) "force_$([System.Guid]::NewGuid()).txt"
      try {
        # Create an existing file and mark read-only
        Set-Content -Path $outFile -Value "OLD"
        $fi = Get-Item $outFile
        $fi.IsReadOnly = $true

        # Without force this should error
        { Invoke-PowerCat -s $tempDir -o $outFile -IncludeMarkdown -ErrorAction Stop } | Should -Throw

        # With force it should overwrite
        Invoke-PowerCat -s $tempDir -o $outFile -IncludeMarkdown -ForceOverwrite
        $content = Get-Content $outFile -Raw
        $content | Should -Match "NEW"
      }
      finally {
        # Ensure file is deletable
        if (Test-Path $outFile) { (Get-Item $outFile).IsReadOnly = $false }
        Remove-Item -Path $outFile -Force -ErrorAction SilentlyContinue
        Remove-Item -Path $tempDir -Recurse -Force -ErrorAction SilentlyContinue
      }
    }
  ...
  Context "Script wrapper parity" {
    BeforeEach {
      $projectRoot = Split-Path -Parent $PSScriptRoot
      $script = Join-Path -Path $projectRoot -ChildPath "scripts/PowerCat.ps1"
      $script = $ExecutionContext.SessionState.Path.GetUnresolvedProviderPathFromPSPath($script)
    }

    It "Script: positional SourceDir + -IncludeMarkdown works" {
      $tempDir = Join-Path ([System.IO.Path]::GetTempPath()) "ScriptPos_$([System.Guid]::NewGuid())"
      New-Item -ItemType Directory -Path $tempDir -Force | Out-Null
      Set-Content -Path (Join-Path $tempDir "a.md") -Value "ScriptPos"
      $outFile = Join-Path ([System.IO.Path]::GetTempPath()) "script_pos_$([System.Guid]::NewGuid()).txt"
      try {
        & pwsh -NoProfile -File $script $tempDir -IncludeMarkdown -o $outFile
        $LASTEXITCODE | Should -Be 0
        (Get-Content $outFile -Raw) | Should -Match 'ScriptPos'
      }
      finally {
    ...
    It "Script: accepts comma-separated -Extensions string" {
      $tempDir = Join-Path ([System.IO.Path]::GetTempPath()) "ScriptComma_$([System.Guid]::NewGuid())"
      New-Item -ItemType Directory -Path $tempDir -Force | Out-Null
      Set-Content -Path (Join-Path $tempDir "one.ps1") -Value "PS1"
      Set-Content -Path (Join-Path $tempDir "two.md") -Value "MD"
      $outFile = Join-Path ([System.IO.Path]::GetTempPath()) "script_comma_$([System.Guid]::NewGuid()).txt"
      try {
        & pwsh -NoProfile -File $script $tempDir -o $outFile -Extensions ".ps1,.md"
        $LASTEXITCODE | Should -Be 0
        $content = Get-Content $outFile -Raw
        $content | Should -Match "PS1"
        $content | Should -Match "MD"
    ...
    It "Script: respects -ExcludeExtensions and -ForceOverwrite" {
      $tempDir = Join-Path ([System.IO.Path]::GetTempPath()) "ScriptForce_$([System.Guid]::NewGuid())"
      New-Item -ItemType Directory -Path $tempDir -Force | Out-Null
      Set-Content -Path (Join-Path $tempDir "one.ps1") -Value "PS1"
      Set-Content -Path (Join-Path $tempDir "two.md") -Value "MD"
      $outFile = Join-Path ([System.IO.Path]::GetTempPath()) "script_force_$([System.Guid]::NewGuid()).txt"
      try {
        # create existing read-only file
        Set-Content -Path $outFile -Value "OLD"
        (Get-Item $outFile).IsReadOnly = $true

        # run without ForceOverwrite: expect non-zero exit code
        & pwsh -NoProfile -File $script $tempDir -o $outFile -IncludeMarkdown
        $LASTEXITCODE | Should -Not -Be 0

        # run with ForceOverwrite: should succeed
        & pwsh -NoProfile -File $script $tempDir -o $outFile -IncludeMarkdown -force
        $LASTEXITCODE | Should -Be 0
        (Get-Content $outFile -Raw) | Should -Match "MD"
      }
      finally {
        if (Test-Path $outFile) { (Get-Item $outFile).IsReadOnly = $false }
        Remove-Item -Path $outFile -Force -ErrorAction SilentlyContinue
